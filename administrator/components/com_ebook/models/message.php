 ...
 * @since       1.6
 */
class EbookModelMessage extends JModelAdmin
{
	/**
	 * Message
	 */
	 ...
	 * @since   1.6
	*/
	public function getTable($type = 'Message', $prefix = 'EbookTable', $config = array())
	{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_ebook.message', 'message', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$this->preprocessData('com_ebook.message', $data);
	...
	public function save($data)
	{
		...
		$model  = JModelLegacy::getInstance('Config', 'EbookModel', array('ignore_request' => true));
		...
		{
			$this->setError(JText::_('COM_EBOOK_ERR_SEND_FAILED'));
			return false;
		}
		...
		if ($config->get('mail_on_new', true))
		{
			...
			$lang->load('com_ebook', JPATH_ADMINISTRATOR);

			// Build the email subject and message
			$sitename = JFactory::getApplication()->get('sitename');
			$siteURL  = JUri::root() . 'administrator/index.php?option=com_ebook&view=message&message_id=' . $table->message_id;
			$subject  = sprintf($lang->_('COM_EBOOK_NEW_MESSAGE_ARRIVED'), $sitename);
			$msg      = sprintf($lang->_('COM_EBOOK_PLEASE_LOGIN'), $siteURL);
